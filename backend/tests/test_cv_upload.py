...
def _upload_pdf(client, user_id: int, content: bytes, content_type: str = 'application/pdf'):
    return client.post(
        '/api/v1/cv/upload',
...
class TestCVUploadEndpoint:
    def test_upload_pdf_creates_profile(self, client, db_session):
        user_svc = UserService(db_session)
        user = user_svc.create_user(email='upload@example.com')
        resp = _upload_pdf(client, user.id, _make_valid_pdf_bytes())
    ...
    def test_rejects_non_pdf_content_type(self, client, db_session):
        user_svc = UserService(db_session)
        user = user_svc.create_user(email='notpdf@example.com')
        resp = _upload_pdf(client, user.id, _make_valid_pdf_bytes(), content_type='text/plain')
        assert resp.status_code == 415
        assert 'Only PDF files are accepted' in resp.json()['detail']

    def test_rejects_non_pdf_magic_bytes(self, client, db_session):
        user_svc = UserService(db_session)
        user = user_svc.create_user(email='badmagic@example.com')
        resp = _upload_pdf(client, user.id, _make_txt_bytes())
        assert resp.status_code == 415
    ...
    def test_enforces_size_limit(self, client, db_session, monkeypatch):
        ...
        user_svc = UserService(db_session)
        user = user_svc.create_user(email='bigfile@example.com')
        big_pdf = b'%PDF-1.4 ' + b'x' * 1000
        resp = _upload_pdf(client, user.id, big_pdf)
        assert resp.status_code == 413
        assert 'exceeds maximum size' in resp.json()['detail']

    def test_duplicate_upload_returns_existing(self, client, db_session):
        user_svc = UserService(db_session)
        user = user_svc.create_user(email='dup@example.com')
        pdf = _make_valid_pdf_bytes()
        first = _upload_pdf(client, user.id, pdf)
        assert first.status_code == 200
        assert first.json()['duplicate'] is False
        second = _upload_pdf(client, user.id, pdf)
    ...
    def test_file_stored_on_disk(self, client, db_session, tmp_path, monkeypatch):
        ...
        user_svc = UserService(db_session)
        user = user_svc.create_user(email='disk@example.com')
        monkeypatch.setattr(settings, 'cv_upload_dir', str(upload_dir))
        pdf_bytes = _make_valid_pdf_bytes()
        resp = _upload_pdf(client, user.id, pdf_bytes)
        assert resp.status_code == 200
    ...
    def test_get_cv_returns_status_and_filename(self, client, db_session):
        user_svc = UserService(db_session)
        user = user_svc.create_user(email='getcv@example.com')
        resp = _upload_pdf(client, user.id, _make_valid_pdf_bytes())
        cv_id = resp.json()['id']
    ...
    def test_get_nonexistent_cv_returns_404(self, client):
        resp = client.get('/api/v1/cv/99999')
    ...
    def test_extraction_failure_sets_status_failed(self, client, db_session, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(settings, 'cv_upload_dir', str(upload_dir))
        user_svc = UserService(db_session)
        user = user_svc.create_user(email='badcv@example.com')
        resp = _upload_pdf(client, user.id, b'%PDF-1.4 corrupted garbage')
    ...
    def test_empty_pdf_sets_status_failed(self, client, db_session, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(settings, 'cv_upload_dir', str(upload_dir))
        user_svc = UserService(db_session)
        user = user_svc.create_user(email='emptypdf@example.com')
        doc = fitz.open()
        doc.new_page(width=612, height=792)
        bio = io.BytesIO()
        doc.save(bio)
        doc.close()
        resp = _upload_pdf(client, user.id, bio.getvalue())
        assert resp.status_code == 422
        assert 'extraction failed' in resp.json()['detail']
    ...
    def test_upload_stores_raw_text(self, client, db_session, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(settings, 'cv_upload_dir', str(upload_dir))
        user_svc = UserService(db_session)
        user = user_svc.create_user(email='rawtext@example.com')
        resp = _upload_pdf(client, user.id, _make_valid_pdf_bytes('Skills: Python, SQL'))
        assert resp.status_code == 200
        cv_id = resp.json()['id']
        get_resp = client.get(f'/api/v1/cv/{cv_id}')
        assert get_resp.status_code == 200
