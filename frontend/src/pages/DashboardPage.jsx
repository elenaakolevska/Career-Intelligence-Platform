    ...
    <ViewShell>
      ...
      <div className="hero">
        ...
        <div>
          ...
          <div className="chips">
            {strong.length > 0 && (
              <span className="chip good">✓ Strong: {strong.join(' · ')}</span>
            )}
            {improve.length > 0 && (
      ...
      {topGap && (
        <div className="card insight" style={{ marginBottom: 16 }}>
      ...
      <div className="grid2">
        <div className="card">
          <div className="cardtitle">
            Your skill profile <span>vs. market demand</span>
          </div>
          {profileRows.length === 0 ? (
            <p className="empty-state">No skill profile yet.</p>
          ) : (
            profileRows.map((row) => (
              <SkillBar
                key={row.name}
        ...
        <div className="card">
          <div className="cardtitle">
            Best opportunities <span>Top {Math.min(3, matches.length)} of {matches.length}</span>
          </div>
          {matches.length === 0 ? (
      ...
      <div className="grid2">
        <div className="card">
          <div className="cardtitle">
            Priority skill gaps <span>{gaps.length} identified</span>
          </div>
          {gaps.length === 0 ? (
            <p className="empty-state">No material gaps detected.</p>
          ) : (
            gaps.slice(0, 3).map((g) => {
              ...
              return (
                ...
                >
                  <i
                    className="dot"
                    style={{ background: p === 'high' ? 'var(--red)' : 'var(--amber)' }}
                  />
                  <div>
        ...
        <div className="card next">
          ...
          <div>
            ...
            <p>
              {topGap
                ? `Focus on ${topGap.skill}: apply it in one existing project this week.`
